...
const emit = defineEmits(['onFormSave'])
...
const props = defineProps({
    formDataJson: String,
    formConfJson: String,
    drawingListJson: String,
    generateConfJson: String
})

// drawingList.value = !props.drawingListJson
//     ? drawingDefalut
//     : JSON.parse(props.drawingListJson)
// formData.value = !props.formDataJson ? {} : JSON.parse(props.formDataJson)
// formConf.value = !props.formConfJson
//     ? formConfData
//     : JSON.parse(props.formConfJson)
// generateConf.value = !props.generateConfJson
//     ? null
//     : JSON.parse(props.generateConfJson)
...
watch(
    ...
    (newVal) => {
        try {
            if (!newVal) return
            formData.value = JSON.parse(newVal)
        } catch (error) {
            console.error('JSON 解析错误:', error)
            parsedData.value = {} // 解析失败，赋默认值
        }
    },
    { immediate: true }
)
...
watch(
    ...
    (newVal) => {
        try {
            if (!newVal) return
            formConf.value = JSON.parse(newVal)
        } catch (error) {
            console.error('JSON 解析错误:', error)
            formConf.value = formConfData // 解析失败，赋默认值
        }
    },
    { immediate: true }
)
...
watch(
    () => props.drawingListJson,
    (newVal) => {
        try {
            if (!newVal) return
            drawingList.value = JSON.parse(newVal)
        } catch (error) {
            console.error('JSON 解析错误:', error)
            drawingList.value = drawingDefalut // 解析失败，赋默认值
        }
    },
    { immediate: true }
)
...
watch(
    () => props.generateConfJson,
    (newVal) => {
        try {
            if (!newVal) return
            generateConf.value = JSON.parse(newVal)
        } catch (error) {
            console.error('JSON 解析错误:', error)
            generateConf.value = null // 解析失败，赋默认值
        }
    },
    { immediate: true }
)

// drawingList.value = computed(() => {
//     if (!props.drawingListJson) return drawingDefalut
//     return JSON.parse(props.drawingListJson)
// })
// formData.value = computed(() => {
//     if (!props.formDataJson) return {}
//     return JSON.parse(props.formDataJson)
// })
// formConf.value = computed(() => {
//     if (!props.formConfJson) return formConfData
//     return JSON.parse(props.formConfJson)
// })

// generateConf.value = computed(() => {
//     if (!props.generateConfJson) return null
//     return JSON.parse(props.generateConfJson)
// })
            ...
            .center-board-row {
                ...
                & > .el-form {
                        ...
                        .active-from-item {
                            ...
                            .el-input__wrapper {
                                box-shadow: 0 0 0 1px
                                    var(--el-input-hover-border-color) inset;
                            }
                        }
                    ...
                    .drawing-item {
                        ...
                        &.unfocus-bordered:not(.activeFromItem)
                            > div:first-child {
